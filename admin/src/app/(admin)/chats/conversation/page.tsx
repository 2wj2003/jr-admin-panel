...
export default function ChatConversationPage() {
    ...
    try {
      ...
      try {
        const response = await api.get(
          `/content-manager/collection-types/api::chat-session.chat-session?filters[sessionId][$eq]=${sessionId}`
        );
        results = response.data?.results || response.data?.data || [];
      } catch {
        // Fallback to Public API
        const response = await api.get(
          `/api/chat-sessions?filters[sessionId][$eq]=${sessionId}`
        );
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      ...
      try {
        const response = await api.get(
          `/content-manager/collection-types/api::chat-message.chat-message?filters[sessionId][$eq]=${sessionId}&sort=createdAt:asc&pagination[pageSize]=100`
        );
        results = response.data?.results || response.data?.data || [];
      } catch {
        // Fallback to Public API
        const response = await api.get(
          `/api/chat-messages?filters[sessionId][$eq]=${sessionId}&sort=createdAt:asc&pagination[pageSize]=100`
        );
        results = (response.data?.data || []).map((item: any) => ({
          id: item.id, ...(item.attributes || item)
        }));
  ...
  }, [sessionId, loadMessages, loadSessionInfo]);
  ...
  const handleCloseSession = async () => {
    try {
      try {
        ...
        if (results.length > 0) {
          await api.put(
            `/content-manager/collection-types/api::chat-session.chat-session/${results[0].id}`,
            { status: "closed" }
          );
        }
      } catch {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">ไม่พบ Session</h1>
        </div>
        <p className="text-muted-foreground">กรุณาเลือกการสนทนาจากรายการแชท</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground mt-1">Session: {sessionId}</p>
        </div>
        {!sessionClosed && (
